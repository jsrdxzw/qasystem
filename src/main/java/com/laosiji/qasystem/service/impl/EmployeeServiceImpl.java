...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    public ResultData register(EmployeeRo employeeRo) {
        ...
        if (!StringUtils.hasText(userName)){
            resultData = ResultData.fail(-1,"用户名不能为空");
            return resultData;
        }
        if (!StringUtils.hasText(passWord)){
            resultData = ResultData.fail(-1,"密码不能为空");
            return resultData;
        }
        if (!StringUtils.hasText(email)){
            resultData = ResultData.fail(-1,"邮箱不能为空");
            return resultData;
        }
        //通过用户名查询
        Employee employee = employeeDao.selectUserByUserName(userName);
        if (employee!=null){
            resultData = ResultData.fail(-1,"该用户名已被其他人使用,请修改用户名");
            return resultData;
        }
        Employee employeeByEmail = employeeDao.selectUserByEmail(email);
        if (employeeByEmail!=null){
            resultData = ResultData.fail(-1,"该邮箱已被其他人使用,请修改邮箱");
            return resultData;
        }
